...
class Menu extends Database{
	...
	public function linkByMenuTypeAlias($menutype, $alias){
		...
		$query = $this->q("SELECT id, title, link, params FROM `$this->menu` WHERE menutype = '$menutype' AND alias = '$alias'");

		//Create variables for each column queried
		$id = $query[0]['id'];
		$title = $query[0]['title'];
		$link = $query[0]['link'];
		$params = $query[0]['params'];
